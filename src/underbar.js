...
(function() {
  ...
  _.map = function(array, iterator) {
    ...
    _.each(array, function(item){
      var result = iterator(item, array);
  ...
  _.invoke = function(collection, functionOrKey, args) {
    if(functionOrKey.length){//Then it's a method
  ...
  _.delay = function(func, wait) {
    ...
    return setTimeout(function(){ return func.apply(this, args)}, wait);//Wait, why pass 'this' ?
  };
  ...
  _.shuffle = function(array) {
    //DOESN'T ACTUALLY "shuffle"
    var newarray = Array.prototype.slice(array);
    var newestarray = [];
    var rand;

    newestarray[0] = newarray.shift();

    _.each(newarray, function(item) {
      rand = Math.floor(Math.random()*(2));
      if(rand === 1){
        newestarray.unshift(item);
      } else if(rand === 2){
        newestarray.push(item);
      }
    });
